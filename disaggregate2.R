disaggregate2 <- function (data) 
{
  ...
  data.new <- cbind(data,  reshape2::colsplit(data$aggr_Fragment_Annotation, 
                                   ";", paste("Split_FragAnnot_", seq(1:max(n.transitions2)), 
                                              sep = "")),  reshape2::colsplit(data$aggr_Peak_Area, ";", paste("Split_PeakArea_", 
                                                                                                   seq(1:max(n.transitions2)), sep = "")), stringsAsFactors = FALSE)
  data.new.m <- reshape2::melt(data.new, id.vars = grep("Split_FragAnnot", 
                                                        colnames(data.new), invert = TRUE), measure.vars = grep("Split_FragAnnot", 
                                                                                                                colnames(data.new)), variable.name = "FragAnnot_N", value.name = "Fragment")
  data.new.m2 <- reshape2::melt(data.new.m, id.vars = grep("Split_PeakArea", 
                                                           colnames(data.new.m), invert = TRUE), measure.vars = grep("Split_PeakArea", 
                                                                                                                     colnames(data.new.m)), variable.name = "Area_N", value.name = "Area")
  ...
  data.new.m3 <- data.new.m2[gsub("Split_FragAnnot_", "", data.new.m2[, 
                                                                      "FragAnnot_N"]) == gsub("Split_PeakArea_", "", data.new.m2[, 
                                                                                                                                 "Area_N"]), ]
  cols <- colnames(data.new.m3)[colnames(data.new.m3) %in% 
                                  c("transition_group_id", "m_score", "ProteinName", "FullPeptideName", "PeptideSequence", 
                                    ...
                                    "FragmentIon", "Area", "Condition", "BioReplicate", 
                                    "Run", "RT")]
